using DB_project.Models.DataValidator;
using DB_project.Models.Tables;
...
namespace DB_project.Models
{
    ...
    {
        public string table_name = AttributeNames.FundamentalInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public FundamentalInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.FundamentalInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.FundamentalInfoAttributeNames[i];
                string type = AttributeNames.FundamentalInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
...
/*    public class ModifiableBasicInfo : ValueFunc
    {
        public Dictionary<string, string> data =
                AttributeNames.ModifiableBasicInfoAttributeNames.ToDictionary(key => key, Value => string.Empty);
    }*/
}
