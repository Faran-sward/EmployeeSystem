...
namespace DB_project.Models
{
    ...
    {
        public string table_name = AttributeNames.BusinessInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public BusinessInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.BusinessInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.BusinessInfoAttributeNames[i];
                string type = AttributeNames.BusinessInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
