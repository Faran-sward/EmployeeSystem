...
namespace DB_project.Models.DataManager
{
    ...
    public class BusinessInfoManager
    {
        ...
        public bool Get(string json, out List<BusinessInfo> ret)
        {
            ...
            {
                BusinessInfo temp = new BusinessInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    ...
                    // 使用 SetValuesThroughOrclReader 函数从 OracleDataReader 中读取一行值并设置到 BusinessInfo 实例中
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                BusinessInfo set_temp = new BusinessInfo(), search_temp = new BusinessInfo();  // 新建表用于存储 json 中属性及值
