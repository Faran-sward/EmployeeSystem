...
namespace EmployeeSystem.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.WorkScheduleTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public WorkSchedule() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.WorkScheduleAttributeNames.Length; i++)
            {
                string name = AttributeNames.WorkScheduleAttributeNames[i];
                string type = AttributeNames.WorkScheduleAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
