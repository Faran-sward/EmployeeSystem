...
namespace EmployeeSystem.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.CommuteTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public Commute() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.CommuteAttributeNames.Length; i++)
            {
                string name = AttributeNames.CommuteAttributeNames[i];
                string type = AttributeNames.CommuteAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
