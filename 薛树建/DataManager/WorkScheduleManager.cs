...
namespace EmployeeSystem.Models.DataManager
{
    ...
    public class WorkScheduleManager
    {
        ...
        {
            ret = new List<WorkSchedule>();
            if (!string.IsNullOrEmpty(json))
            {
                WorkSchedule temp = new WorkSchedule();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    ...
                    if (workschedule.SetValuesThroughOrclReader(reader))
                    {
                        Console.WriteLine("read successfully! ");
                        ret.Add(workschedule);
                    }
                    else
                        break;
                }
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                WorkSchedule set_temp = new WorkSchedule(), search_temp = new WorkSchedule();  // 新建表用于存储 json 中属性及值
