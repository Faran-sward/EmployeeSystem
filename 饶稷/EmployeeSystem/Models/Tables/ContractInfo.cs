...
namespace EmployeeSystem.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.ContractInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public ContractInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.ContractInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.ContractInfoAttributeNames[i];
                string type = AttributeNames.ContractInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
