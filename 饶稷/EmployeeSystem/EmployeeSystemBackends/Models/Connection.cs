...
namespace EmployeeSystemBackends.Models.Connection  // 负责数据库连接
{
    ...
    public class DatabaseLink : IDisposable  // 负责数据库的连接类
    {
        ...
        private bool SetConnectionString(string hostname, string port, string service_name, string user_id, string password)
        {
        ...
        public DatabaseLink()
        {
            try
            {
                if (!SetConnectionString("localhost", "1521", "orcl", "TESTER", "123456"))
                    Console.WriteLine("connection failed ! ");
                connection = new OracleConnection(connection_string);
                Connect();
            }
