...
namespace EmployeeSystemBackends.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.PostInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public PostInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.PostInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.PostInfoAttributeNames[i];
                string type = AttributeNames.PostInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
