...
namespace EmployeeSystemBackends.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.EmpSalaryTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public EmpSalary() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.EmpSalaryAttributeNames.Length; i++)
            {
                string name = AttributeNames.EmpSalaryAttributeNames[i];
                string type = AttributeNames.EmpSalaryAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
