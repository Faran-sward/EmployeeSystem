...
namespace EmployeeSystemBackends.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.EducationInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public EducationInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.EducationInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.EducationInfoAttributeNames[i];
                string type = AttributeNames.EducationInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
