...
namespace EmployeeSystemBackends.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.ExperienceInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public ExperienceInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.ExperienceInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.ExperienceInfoAttributeNames[i];
                string type = AttributeNames.ExperienceInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
