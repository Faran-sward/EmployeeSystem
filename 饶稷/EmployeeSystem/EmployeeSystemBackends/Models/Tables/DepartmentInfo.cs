...
namespace EmployeeSystemBackends.Models.Tables
{
    ...
    {
        public string table_name = AttributeNames.DepartmentInfoTableName;

        private Dictionary<string, string?> values = new Dictionary<string, string?>();

        private Dictionary<string, string> types = new Dictionary<string, string>();

        public DepartmentInfo() // 初始化字典 types
        {
            for (int i = 0; i < AttributeNames.DepartmentInfoAttributeNames.Length; i++)
            {
                string name = AttributeNames.DepartmentInfoAttributeNames[i];
                string type = AttributeNames.DepartmentInfoAttributeTypes[i];
                types[name] = type;
                values[name] = null;
            }
        }
