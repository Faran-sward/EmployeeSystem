...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class EducationInfoManager
    {
        ...
        public bool Get(string json, out List<EducationInfo> ret)
        {
            ret = new List<EducationInfo>();
            if (!string.IsNullOrEmpty(json))
            {
                EducationInfo temp = new EducationInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    EducationInfo contract = new EducationInfo();
                    // 使用 SetValuesThroughOrclReader 函数从 OracleDataReader 中读取一行值并设置到 EducationInfo 实例中
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                EducationInfo set_temp = new EducationInfo(), search_temp = new EducationInfo();  // 新建表用于存储 json 中属性及值
