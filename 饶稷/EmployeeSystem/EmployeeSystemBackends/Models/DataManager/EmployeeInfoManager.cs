...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class EmployeeInfoManager
    {
        ...
        //private static string connectionString = "Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=8.130.119.222)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=ORCL)));Persist Security Info=True;User ID=TESTER;Password=123456;";
        // 根据JSON信息，从数据库获取对应值，默认返回整个元组
        public bool Get(string json, out List<EmployeeInfo> ret)
        {
            ret = new List<EmployeeInfo>();
            if (!string.IsNullOrEmpty(json))
            {
                EmployeeInfo temp = new EmployeeInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    ...
                    if (employee.SetValuesThroughOrclReader(reader))
                    {
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                EmployeeInfo set_temp = new EmployeeInfo(), search_temp = new EmployeeInfo();  // 新建表用于存储 json 中属性及值
