...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class ExperienceInfoManager
    {
        ...
        public bool Get(string json, out List<ExperienceInfo> ret)
        {
            ret = new List<ExperienceInfo>();
            if (!string.IsNullOrEmpty(json))
            {
                ExperienceInfo temp = new ExperienceInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    ExperienceInfo contract = new ExperienceInfo();
                    // 使用 SetValuesThroughOrclReader 函数从 OracleDataReader 中读取一行值并设置到 ExperienceInfo 实例中
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                ExperienceInfo set_temp = new ExperienceInfo(), search_temp = new ExperienceInfo();  // 新建表用于存储 json 中属性及值
