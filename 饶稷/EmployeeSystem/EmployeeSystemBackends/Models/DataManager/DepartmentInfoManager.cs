...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class DepartmentInfoManager
    {
        ...
        public bool Get(string json, out List<DepartmentInfo> ret)
        {
            ret = new List<DepartmentInfo>();
            if (!string.IsNullOrEmpty(json))
            {
                DepartmentInfo temp = new DepartmentInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    DepartmentInfo contract = new DepartmentInfo();
                    // 使用 SetValuesThroughOrclReader 函数从 OracleDataReader 中读取一行值并设置到 DepartmentInfo 实例中
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                DepartmentInfo set_temp = new DepartmentInfo(), search_temp = new DepartmentInfo();  // 新建表用于存储 json 中属性及值
