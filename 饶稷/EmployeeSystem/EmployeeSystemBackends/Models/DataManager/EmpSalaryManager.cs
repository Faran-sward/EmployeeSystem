...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class EmpSalaryManager
    {
        ...
        public bool Get(string json, out List<EmpSalary> ret)
        {
            ret = new List<EmpSalary>();
            if (!string.IsNullOrEmpty(json))
            {
                EmpSalary temp = new EmpSalary();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    EmpSalary contract = new EmpSalary();
                    // 使用 SetValuesThroughOrclReader 函数从 OracleDataReader 中读取一行值并设置到 EmpSalary 实例中
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                EmpSalary set_temp = new EmpSalary(), search_temp = new EmpSalary();  // 新建表用于存储 json 中属性及值
