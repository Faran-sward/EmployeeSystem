...
namespace EmployeeSystemBackends.Models.DataManager
{
    ...
    public class PostInfoManager
    {
        ...
        public bool Get(string json, out List<PostInfo> ret)
        {
            ret = new List<PostInfo>();
            if (!string.IsNullOrEmpty(json))
            {
                PostInfo temp = new PostInfo();  // 新建表用于存储 json 中属性及值
                ...
                while (true)  // 循环读入
                {
                    PostInfo contract = new PostInfo();
        ...
        public bool Update(string set_json, string search_json)
        {
            ...
            {
                PostInfo set_temp = new PostInfo(), search_temp = new PostInfo();  // 新建表用于存储 json 中属性及值
