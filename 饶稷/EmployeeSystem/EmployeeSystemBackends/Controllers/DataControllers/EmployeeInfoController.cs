    ...
    public class EmployeeInfoController : ControllerBase
    {
        ...
        public IActionResult Get(string json)
        {
            try
            {
                ...
                EmployeeInfoManager manager = new EmployeeInfoManager();  // 负责与数据库交互的manager
                if (!manager.Get(json, out List<EmployeeInfo> tables))
                    return StatusCode(500, $"读取数据失败:");  // 获取数据存放在table中

                List<object> jsonResults = new List<object>();

                foreach (EmployeeInfo table in tables)
                {
