...
void Resume::presentEmployeeInfo() {
    int index = ui->comboBox->currentIndex();
    // 获取选定的部门名称
    QString selectedDepartment = ui->comboBox->itemText(index);
    ...
    QObject::connect(reply, &QNetworkReply::finished, [=]() {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            // 连接表格的点击事件与更新TextEdit内容的槽函数
            QObject::connect(ui->employeeTable, &QTableWidget::cellClicked, this, &Resume::onEmployeeCellClicked);
        }
        else {
            // 请求失败
            qDebug() << "请求失败" << reply->errorString();
...
void Resume::onEmployeeCellClicked(int row, int column) {
    // 获取点击的EmployeeID
    QTableWidgetItem* item = ui->employeeTable->item(row, column);
    QString selectedEmployeeID = item->text();

    // 更新TextEdit的内容为所选EmployeeID
    ui->rsmSearchID->setText(selectedEmployeeID);
}

void Resume::treatCheckedCombo(){
    //呈现部门信息
    presentDepartInfo();
...
void Resume::Leftmenu_Animation(){
