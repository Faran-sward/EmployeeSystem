...
MyInfomation::MyInfomation(QString employeeID,QWidget *parent) :
    QWidget(parent),
...
//基本信息呈现
void MyInfomation::basicInfomation(){
...
void MyInfomation::dealBasicInfo(QJsonObject jsonObj){
    QJsonObject basicInfoObj = jsonObj["BasicInfo"].toObject();

    Employee tempEmployee(basicInfoObj);
...
void MyInfomation::dealEduInfo(QJsonObject jsonObj){
    // 提取"ResumeInfo"
    ...
    // 存储"ResumeInfo"的对象数组
    QVector<ResumeInfo> resumeInfos;
...
void MyInfomation::dealSalaryInfo(QJsonObject jsonObj){
    // 提取"SalaryInfo"
    ...
    // 存储"ResumeInfo"的对象数组
    QVector<SalaryInfo> salaryInfos;
...
//窗口动画
void MyInfomation::applyAnimation(){
...
//退出键对应的动画
void MyInfomation::exitAnimation(){
